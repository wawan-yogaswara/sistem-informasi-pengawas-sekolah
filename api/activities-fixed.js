...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      if (!additionalTasksError && additionalTasksData) {
        ...
        if (activities.length > 0) {
          console.log('➕ Sample activity data:', {
            ...
            photo: activities[0].photo ? 'EXISTS' : 'NULL',
            photo1: activities[0].photo1 ? 'EXISTS' : 'NULL',
            photo2: activities[0].photo2 ? 'EXISTS' : 'NULL',
      ...
      const { title, description, date, user_id, type, location, name, organizer, photo, photo2 } = req.body;
      ...
      if (error) {
        ...
        return res.status(500).json({ error: 'Failed to update activity', details: error.message });
      }
      ...
      if (error) {
        ...
        return res.status(500).json({ error: 'Failed to delete activity', details: error.message });
      }
